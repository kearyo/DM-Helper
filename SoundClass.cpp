...
IDirectSoundBuffer8* SoundClass::InitSoundBuffer(CString szWAVFileName)
{
	IDirectSoundBuffer8* pBuffer = NULL;

	// Load a wave audio file onto a secondary buffer.
	HRESULT result = LoadWaveFile(szWAVFileName.GetBuffer(0), &pBuffer);
	if (!result)
	{
		return NULL;
	}
...
{
	for (POSITION pos = m_DXSoundBufferMap.GetStartPosition(); pos != NULL;)
	{
		CString szString = _T("");
		PSOUNDBUFFER pDXSoundBuffer = NULL;
		m_DXSoundBufferMap.GetNextAssoc(pos, szString, pDXSoundBuffer);

		if (pDXSoundBuffer != NULL)
		{
			ShutdownWaveFile(&pDXSoundBuffer);
			delete pDXSoundBuffer;
		}
	}
	m_DXSoundBufferMap.RemoveAll();
...
bool SoundClass::LoadWaveFile(char* filename, IDirectSoundBuffer8** secondaryBuffer)
{
	...
	int nChannels = waveFileHeader.numChannels;
	...
	int nSampleRate = waveFileHeader.sampleRate;
...
bool SoundClass::PlaySoundBuffer(IDirectSoundBuffer8* pBuffer)
{
	...
	result = pBuffer->SetVolume(DSBVOLUME_MAX);
	if (FAILED(result))
	{
		return false;
	}
...
bool SoundClass::PlayDXSoundFromFile(CString szWAVPath)
{
	PSOUNDBUFFER pDXSoundBuffer = NULL;
	if (m_DXSoundBufferMap.Lookup(szWAVPath, pDXSoundBuffer) == FALSE || pDXSoundBuffer == NULL)
	{
		pDXSoundBuffer = InitSoundBuffer(szWAVPath);
		m_DXSoundBufferMap.SetAt(szWAVPath, pDXSoundBuffer);
	}

	if (pDXSoundBuffer != NULL)
	{
		return PlaySoundBuffer(pDXSoundBuffer);
	}

	return false;
}

int SoundClass::GetBufferedSoundCount()
{
	return m_DXSoundBufferMap.GetCount();
}
